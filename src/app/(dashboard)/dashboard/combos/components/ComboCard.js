...
function resolveModelString(m) {
  if (typeof m === "string") return m;
  if (m && typeof m === "object") {
    return m.model || m.id || m.name || (m.provider && m.model ? (m.provider + "/" + m.model) : JSON.stringify(m));
  }
  return String(m || "");
}
...
export default function ComboCard({ combo, modelCaps = {}, activeProviders = [], copied, onCopy, onEdit, onDelete, strategy = {}, onSetStrategy }) {
  ...
  const { hasEffort, hasExtended, hasMaxEffort, unresolvableModels, unsupportedForCurrent } =
    classifyComboThinking(models.map(model => ({ model, caps: modelCaps[resolveModelString(model)] || {} })), thinkingType);
              ...
              {models.length === 0 ? (
                ...
                models.slice(0, 3).map((model, index) => (
                  <code key={index} className="inline-flex items-center gap-1 rounded bg-black/5 px-1.5 py-0.5 font-mono text-xs text-text-muted dark:bg-white/5">
                    <span className="truncate max-w-[120px]">{resolveModelString(model)}</span>
                    {modelCaps[resolveModelString(model)] && <CapacityBadges caps={modelCaps[resolveModelString(model)]} size={11} />}
                  </code>
                ))
              )}
              {models.length > 3 && (
            ...
            <div className="flex flex-col gap-1">
              {models.map((model, index) => (
                <div key={index} className="flex items-center gap-2 rounded px-2 py-1 bg-black/[0.02] dark:bg-white/[0.02]">
                  ...
                  <code className="min-w-0 flex-1 truncate font-mono text-xs text-text-main">{resolveModelString(model)}</code>
                  {modelCaps[resolveModelString(model)] && <CapacityBadges caps={modelCaps[resolveModelString(model)]} size={11} />}
                </div>
              ))}
            </div>
              ...
              {unsupportedForCurrent.length > 0 && (
                <div className="flex items-center gap-1 text-amber-600 dark:text-amber-400">
                  ...
                  <span className="text-[10px]">
                    {thinkingType} not supported by {unsupportedForCurrent.map(m => (typeof m?.model === "string" ? m.model : String(m?.model || "")).split("/").pop()).join(", ")}
                  </span>
                </div>
              )}
